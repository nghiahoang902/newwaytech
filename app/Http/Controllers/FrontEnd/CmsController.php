...
class CmsController extends Controller
{
    ...
    {
        return $this->responseView('frontend.intro.index');
    }

    //thiet-bi
    public function device() 
    {
        $this->responseData['taxonomy_thietbi'] = ContentService::getCmsTaxonomy(['taxonomy'=>'thiet-bi','status' => 'active', 'order_by' => ['iorder' => 'ASC']])->first();

        return $this->responseView('frontend.device.index');
    }

    public function detail($alias = null) 
    {
        $product = ContentService::getProducts(['status' => 1, 'alias' => $alias])->first();
        $taxonomy = $product->cmsTaxonomy;

        $this->responseData['product'] = $product;
        $this->responseData['taxonomy'] = $taxonomy;

        // product relate
        $relatedProducts = CmsProduct::where('taxonomy_id', $taxonomy->id)
            ->where('id', '!=', $product->id)
            ->paginate(6);

        $this->responseData['relatedProducts'] = $relatedProducts;

        return $this->responseView('frontend.device.detail');
    }

    //phan-mem
    public function software($alias = null) 
    {
        $aliasStr = str_replace('.html', '', $alias);
        $parentTaxonomy = ContentService::getCmsTaxonomy(['url_part' => $aliasStr, 'status' => 'active'])->first();

        //lấy tất cả taxonomy con theo taxonomy cha
        $childTaxonomyIds = CmsTaxonomy::where('parent_id', $parentTaxonomy->id)
            ->pluck('id')
            ->toArray();

        // nếu có taxonomy con
        if($childTaxonomyIds) {
            //lấy tất cả product theo id taxonomy cha
            $products = CmsProduct::whereIn('taxonomy_id', $childTaxonomyIds)
                ->paginate(6);
    
            $this->responseData['products'] = $products;
        } else {

            $products = $parentTaxonomy->products()->paginate(6);
            $this->responseData['products'] = $products;
        } 

        return $this->responseView('frontend.software.index');
    }

    //lien-he
    public function contact() 
    {
        return $this->responseView('frontend.contact.index');
    }
    ...
    public function sendEmail(Request $request) 
    {
        try {
            ...
            Mail::to('newwaytech.lamdt@gmail.com')->send(new ContactMail($details));
    
            return redirect()->route('frontend.contact.index')->with('success', 'Email đã được gửi đi!');
        } catch (Exception $e) {
            Log::error('Gửi email thất bại: ' . $e->getMessage());
            return back()->withInput()->withErrors(['errors' => 'Gửi email thất bại, vui lòng thử lại sau.']);
        }    
    }
    ...
    //coupons
    public function getCoupon($alias = null, Request $request) {
        if ($alias !="") {
            $params['url_part'] = str_replace('.html','',$alias);
            $params['status'] = Consts::POST_STATUS['active'];
            $detail = ContentService::getDiscountCode($params)->first();
