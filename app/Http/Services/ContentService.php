...
{
    //lấy đanh sách discount
    public static function getDiscountCode($params, $isPaginate = false)
    {
        $query = Coupon::select('coupons.*')
            ...
            ->when(!empty($params['keyword']), function ($query) use ($params) {
                ...
                return $query->where(function ($where) use ($keyword) {
                    return $where->where('coupons.code', 'like', '%' . $keyword . '%')
                        ->orWhere('coupons.name', 'like', '%' . $keyword . '%')
                        ->orWhere('coupons.type', 'like', '%' . $keyword . '%');
                });
            })
            
            ->when(!empty($params['discount_amount']), function ($query) use ($params) {
                return $query->where('coupons.discount_amount', '!=', $params['discount_amount']);
            })
            ->when(!empty($params['min_amount']), function ($query) use ($params) {
                return $query->where('coupons.min_amount', $params['min_amount']);
            })
            ->when(!empty($params['starts_at']), function ($query) use ($params) {
                return $query->where('coupons.starts_at', $params['starts_at']);
            })
            ->when(!empty($params['expires_at']), function ($query) use ($params) {
                return $query->where('coupons.expires_at', $params['expires_at']);
            });

        if (!empty($params['status'])) {
            $query->where('coupons.status', $params['status']);
        } else {
            $query->where('coupons.status', "!=", Consts::STATUS_DELETE);
        }
        ...
        if (!empty($params['order_by'])) {
				if (is_array($params['order_by'])) {
					foreach ($params['order_by'] as $key => $value) {
						$query->orderBy('coupons.' . $key, $value);
					}
				} else {
					$query->orderByRaw('coupons.' . $params['order_by'] . ' desc');
				}
			} else {
				$query->orderByRaw('coupons.id DESC');
			}
    ...
    public static function getImages($params) {
        $query = Images::select('tb_images.*')
            ->when(!empty($params['keyword']), function ($query) use ($params) {
                $keyword = $params['keyword'];
                return $query->where(function($where) use ($keyword) {
                    return $where->where('tb_images.title', 'like', '%' . $keyword . '%');
                });
            });
        return $query;
    }
    ...
    {
        return Option::where('is_system_param', 1)->get();
    }
    ...
    {
        $query = Order::select('tb_orders.*')
            ->selectRaw('SUM(tb_order_details.quantity) AS total_quantity, 
                SUM(tb_order_details.quantity * tb_order_details.price) AS total_money')
